...
impl<T : PartialOrd + Clone> LinkedList<T> {
    ...
    pub fn merge(mut list_a:LinkedList<T>, mut list_b:LinkedList<T>) -> Self
    {
        ...
        while a_index < list_a.length && b_index < list_b.length {
            let a_value = list_a.get(a_index as i32).unwrap();
            let b_value = list_b.get(b_index as i32).unwrap();
            if a_value < b_value {
                list.add(a_value.clone());
                a_index += 1;
            }else {
                list.add(b_value.clone());
                b_index += 1;
            }
        }
        if b_index < list_b.length {
            for i in b_index..list_b.length {
                let b_value = list_b.get(i as i32).unwrap();
                list.add(b_value.clone());
            }
        }
        if a_index < list_a.length {
            for i in a_index..list_a.length {
                list.add(list_a.get(i as i32).unwrap().clone());
            }
        }
        list
    }
